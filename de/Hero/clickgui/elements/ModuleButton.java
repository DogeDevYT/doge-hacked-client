...
public class ModuleButton {
	...
	public ModuleButton(Module imod, Panel pl) {
		 ...
		 */
		if (Doge.instance.settingsManager.getSettingsByMod(imod) != null)
			for (Setting s : Doge.instance.settingsManager.getSettingsByMod(imod)) {
				if (s.isCheck()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if(Doge.instance.settingsManager.getSettingByName("Sound").getValBoolean())
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Doge.instance.clickGui.closeAllSettings();
				this.extended = b;
	...
	public boolean keyTyped(char typedChar, int keyCode) throws IOException {
		...
		if (listening) {
			if (keyCode != Keyboard.KEY_ESCAPE) {
				...
				ChatUtil.info(" Bounded " + EnumChatFormatting.GREEN + mod.getName() + "" + EnumChatFormatting.GRAY + " to " + EnumChatFormatting.GOLD + Keyboard.getKeyName(keyCode) + "");
				mod.setKey(keyCode);
			} else {
				//Client.sendChatMessage("Unbound '" + mod.getName() + "'");
				ChatUtil.info(" Unbounded " + EnumChatFormatting.RED + mod.getName() + "");
